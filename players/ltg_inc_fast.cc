...
void DoWork() {
  // TODO(keisuke): We should select the youngest slot which is alive.
  int youngest_alive = 0;
  // Assumes youngest_alive: I
  _(youngest_alive, ZERO);
  for (int i = 0; i < 256; ++i) {
    // Assumes base: I
    int base = rand() % 256;
    while (base == youngest_alive) base = rand() % 256;
    _(base, INC);
    CallWithSlot(base, youngest_alive);
    _(SUCC, youngest_alive);
  }
  _(PUT, youngest_alive);
}
